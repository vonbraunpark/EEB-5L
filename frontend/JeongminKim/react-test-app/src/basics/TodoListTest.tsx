            ...
            */}
            <div className="bg-white shadow-2xl rounded-3xl w-full max-w-xl p-8">
                ...
                */}
                ...
                {/*
                    flex <- 가로 정렬(input과 button을 나란히)
                    items-center <- 수직 정렬
                    gap-3 <- input과 button 사이에 간격을 12px
                    mb-6 <- 아래 방향 마진 24px
                */}
                <div className="flex items-center gap-3 mb-6">
                    {/*
                        flex-grow <- 어떤 공간 크기가 저장되어 있고 남는 부분이 있으면 전부 채움
                        px-4 <- 좌우 16px 패딩
                        py-2 <- 상하 8px 패딩
                    ...
                    */}
                    ...
                    {/*
                        ...
                        py-2 <- 상하 패딩 2px
                        rounded-lg <- 약간 둥글게
                        hober:bg-blue-700 <- 커서가 위쪽 배치(hover) 되면 진한 파란색 처리
                        active:scale-95 <- 클릭 시 약간 축소
                        transition <- hover / active 효과 부드럽게 처리하도록 지원
                    */}
                    <button
                        onClick={ handleAddTask }
                        className="shrink-0 bg-blue-600 text-white px-2 py-2 rounded-lg text-xl font-medium hover:bg-blue-700 active:scale-95 transition"
                    >
                        추가
                    </button>
                </div>
                ...
                <ul className="space-y-2">
                    { taskList.map((task) => (
                        ...
                        >
                            <span>{ task.text }</span>
                            {/*
                                text-red-500 <- 가장 무난한 기본 빨간색
                                hover: text-red-700 <- hover 시 진한 빨간색
                                transition <- 색상 변화를 부드럽게 전환할 수 있도록 지원
                            */}
                            <button
