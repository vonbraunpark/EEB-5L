export const SecondProblemVersionOne = () => {
    ...
    let profanityUserHashMap : Record<string, number> = {}
    for (let i = 0; i < abuseReportList.length; i++) {
        if (abuseReportList[i].type === "profanity") {
            profanityUserHashMap[abuseReportList[i].userId] = (profanityUserHashMap[abuseReportList[i].userId] || 0) + 1
        }
    }

    let precautionsHashMapRecord :Record<string, number> = {}
