...
public class KakaoAuthenticationRepositoryImpl implements KakaoAuthenticationRepository {
    ...
    //아래 파트를 보면 clientId, redirectUri, tokenRequestUri 등등의 정보를 가지고 있는 것을 볼 수 있다
    //이 정보들은 .env에 적혀있는 내용을 application.yaml에서 인식하여
    ...
    //KAKAO_CLIENT_ID는 .env에 지정되어 있던 정보에 해당합니다
    ...
    //autorize에 의해 KAKAO 벤더로 부터 수신한 인증 code를 가지고  access token 을 요청하는 과정입니다
    public Map<String, Object> getAccessToken(String code) {
        //MultiValueMap <- Key, Value를 여러 다발로 받기 위해 사용
        //Kakao 벤더에서 요구하는 형식에 맞추기 위한 형식 데이터를 구성함
        //http://developers.kakao.com/docs/latest/ko/kakaologin/rest-api#request-token <- 형식
        ...
        //헤더를 구성
        //위 링크에서 요청 헤더 에 해당하는 부분
        //Content-Type: application/x-www-form-urlencoded으로 되어 있음
        ...
        //위에서 Body를 만들었고 그 하단에 요청 헤더가 있으니 그냥 두개를 가지고 HttpEntity를 구성
        HttpEntity<MultiValueMap<String, String>> entity = new HttpEntity<>(formData, headers);

        //restTemplate을 사용하는 순간은 Spring이 Client로서 외부 서버에게 요청하는 상황입니다
        //restTemplate.exchange()는 HTTP 요청을 보내고 응답을 받는 메서드 입니다.
        //tokenRequestUri는 kakao에게 토큰을 요청할 떄 사용하는 URI 주소
        //그리고 위에서 만들어놓은 HttpEntity를 쏘아 보냄
        //4번째 인자는 결과를 Map 형태의 key, Value로 받겠다는 의미입니다
        ResponseEntity<Map> response = restTemplate.exchange(
                tokenRequestUri, HttpMethod.POST, entity, Map.class);

        //Header 등등 관심없으니 응답 Body만 주세요
        return response.getBody();
    }

    @Override
    public Map<String, Object> getUserInfo(String accessToken) {
        //GET/POST http://kapi.kakao.com/v2/user/me
        //요청: 에세스 토큰 방식 헤더 파트를 보면
