...
public interface SampleBoardRepository extends JpaRepository<SampleBoard, Long> {
    ...
    @Query("SELECT b FROM SampleBoard b JOIN FETCH b.writer ORDER BY b.sampleBoardId DESC")
    Page<SampleBoard> findAllWithWriter(Pageable pageable);

    @Query("SELECT b FROM SampleBoard b JOIN FETCH b.writer WHERE b.sampleBoardId = :sampleBoardId")
    Optional<SampleBoard> findByWIthWriter(Long sampleBoardId);
